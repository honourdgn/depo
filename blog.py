...
class RegisterForm(Form):
    name= StringField("isim soyisim",validators=[validators.length(min=3,max=20)])
    username= StringField("kullanıcı adı",validators=[validators.length(min=3,max=35)])
    email= StringField("email adresi",validators=[validators.Email(message="geçerli bir email adresi girilmedi")])
    password=PasswordField("parola :",validators=[
        validators.DataRequired(message="parola yazılması zorunludur"),
...
def delete(id):
    ...
    if result>0:
        ...
        flash("böyle bir makale yok yada böyle bir hakkınız yok","danger")
        return redirect(url_for("index"))
...
def update(id):
    if request.method=="GET":
        ...
        if result==0:
            flash("böyle bir makale yok yada yetkiniz yok.")
            return redirect(url_for("index"))
        else:
